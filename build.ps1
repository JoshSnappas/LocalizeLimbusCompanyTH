...
Copy-Item -Path Localize/TH $BIE_LLC_Path/Localize -Force -Recurse
...
New-Item -Path "$BIE_OTA_LLC_Path/Localize" -Name "TH" -ItemType "directory" -Force
# Copy the changed files to the release directory
foreach ($file in $changedFilesList) {
    if (Test-Path -Path $file) {
        $destination = "$BIE_OTA_LLC_Path/Localize/TH/$file"
        $destination = $destination.Replace("Localize/TH/Localize/TH/", "Localize/TH/")
        $destinationDirectory = Split-Path -Path $destination -Parent
        if (!(Test-Path -Path $destinationDirectory)) {
            New-Item -ItemType Directory -Force -Path $destinationDirectory
